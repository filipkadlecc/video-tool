...
\`\`\`tsx
{
  ...
  border: \`2px solid \${isActive ? COLORS.orange : "rgba(255,255,255,0.08)"}\`,
  borderRadius: 40,
  padding: "16px 36px",
  fontSize: 28,
  fontWeight: 600,
...
Wire all scenes together using \`TransitionSeries\` with \`fade()\` transitions BETWEEN scenes only — no opening or closing fade from/to black:

\`\`\`tsx
const MainComposition: React.FC = () => {
  return (
    <TransitionSeries>
      {/* Scene 1 — starts fully visible at frame 0, NO fade-in from black */}
      ...
      {/* Final scene — ends fully visible on its last frame, NO fade-out to black */}
...
Rules:
- **NEVER add a \`BlackScreen\` (or any black/empty) opening sequence.** The video must start with the first scene's content fully visible on frame 0.
- **NEVER add a \`BlackScreen\` (or any black/empty) closing sequence.** The video must end with the final scene's content fully visible on its last frame.
- **Do NOT add any \`fade()\` transition before the first scene or after the last scene.** Fades belong BETWEEN scenes only.
- **Scenes themselves must not fade in from opacity 0 on their very first frame if they are the first scene, and must not fade out to opacity 0 on their very last frame if they are the last scene.** Reveals must use motion (scale, translate, blur), color, or content-in-content techniques — not a full-screen opacity ramp from 0.
- Add \`+ TRANSITION\` to each scene's \`durationInFrames\` (except the last) to account for the overlap with the next scene's fade.
...
14. \`TransitionSeries\` with fade transitions BETWEEN scenes only — NO BlackScreen bookends, NO opening fade-in from black, NO closing fade-out to black
