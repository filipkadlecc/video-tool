...
export async function POST(request: Request) {
  ...
  const systemPrompt = buildSystemPrompt(animationType, projectSettings, assetPaths, videoContext, styleMode, customTheme);
  ...
  const anthropicMessages: Anthropic.MessageParam[] = messages.map((msg, i) => {
    ...
    if (isLastUser) {
      let userContent = buildUserMessage(msg.content, currentCode, notionContent, scriptWithTimestamps, animationType, currentTapeDurationMs);
